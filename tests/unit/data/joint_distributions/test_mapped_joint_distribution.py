...
from conftest import (
    LinearTargetFunction,
    QuadraticTargetFunction,
    LinearTargetFunctionConfig,
    QuadraticTargetFunctionConfig,
)
...
@pytest.mark.parametrize("target_cls", [LinearTargetFunction, QuadraticTargetFunction])
def test_initialization(gaussian_base, target_cls):
    cfg_tf = (
        LinearTargetFunctionConfig(input_shape=gaussian_base.input_shape)
        if target_cls is LinearTargetFunction
        else QuadraticTargetFunctionConfig(input_shape=gaussian_base.input_shape)
    )
    ...
    assert isinstance(dist.target_function, target_cls)

@pytest.mark.parametrize("target_cls", [LinearTargetFunction, QuadraticTargetFunction])
def test_sample_shape(gaussian_base, target_cls):
    cfg = MappedJointDistributionConfig(
        distribution_config=GaussianConfig(
            input_shape=gaussian_base.config.input_shape, mean=0.0, std=1.0
        ),
...
@pytest.mark.parametrize("target_cls", [LinearTargetFunction, QuadraticTargetFunction])
def test_target_function_output(gaussian_base, target_cls):
    cfg = MappedJointDistributionConfig(
        distribution_config=GaussianConfig(
            input_shape=gaussian_base.config.input_shape, mean=0.0, std=1.0
        ),
