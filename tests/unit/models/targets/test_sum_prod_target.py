...
from src.models.targets.simple_functions import SumProdTarget
...
def test_sum_prod_target_computes_expected():
    ...
    func = SumProdTarget(
        SumProdTargetConfig(input_shape=torch.Size([4]), indices_list=[[0, 1], [2, 3]])
    )
    result = func(X)
    unscaled = torch.tensor([[1.0 * 2.0 + 3.0 * 4.0], [0.5 * 4.0 + -2.0 * 1.0]])
