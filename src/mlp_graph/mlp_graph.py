...
class MlpActivationGraph:
        ...
        with torch.no_grad():
            for layer_idx, (weight, bias) in enumerate(
                ...
                for neuron_idx in range(weight.size(0)):
                    ...
                    configurations = self._evaluate_node(
                        layer_idx,
                        neuron_idx,
                        ancestors,
                        dtype=dtype,
    ...
    def _evaluate_node(
        ...
        *,
        ...
        for assignment in product([-1, 1], repeat=len(ancestors)):
            input_vector = torch.zeros(self.input_dim, dtype=dtype)
            for idx, value in zip(ancestors, assignment):
                input_vector[idx] = float(value)
            layer_outputs = self._forward(input_vector)
            target_values = layer_outputs[layer_index - 1]
            value_tensor = target_values[neuron_index]
            value = float(value_tensor.item())
            configuration = {
                "inputs": {str(idx): int(value) for idx, value in zip(ancestors, assignment)},
                "value": value,
            }
    ...
    def _forward(self, inputs: torch.Tensor) -> List[torch.Tensor]:
        ...
        for layer_idx, (weight, bias) in enumerate(
            ...
            pre_activation = torch.mv(weight, prev)
            if bias is not None:
