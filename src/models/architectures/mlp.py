...
class MLP(Model):
    ...
    # gradient utilities
    # ------------------------------------------------------------------
    def export_neuron_input_gradients(self, data_provider: "DataProvider", path: Path) -> None:
        """Compute average absolute input gradients for each neuron and save CSV.

        Parameters
        ----------
        data_provider:
            A :class:`DataProvider` yielding batches of ``(inputs, targets)``.
            Only the inputs are used.
        path:
            Destination file for the CSV output. Layer indices in the resulting
            CSV are 1-based, with the input layer omitted.
        """
    ...
    # visualization
    # ------------------------------------------------------------------
    def visualize(self, path: Path, threshold: float = 0.001) -> None:
        """Visualize the network architecture and save as a PNG image.

        Parameters
        ----------
        path:
            Directory containing ``neuron_input_gradients.csv`` and where the
            visualization will be saved as ``visualization.png``.
        threshold:
            Minimum absolute weight magnitude required to draw an edge.
        """
        ...
        # ------------------------------------------------------------------
        # collect gradient information for hidden layer ordering
        # ------------------------------------------------------------------
        ...
        # maximise horizontal spread while respecting a 2:1 aspect ratio
        y_spacing = 1.0
        ...
        # ------------------------------------------------------------------
        # determine neuron permutations based on gradient magnitudes
        # ------------------------------------------------------------------
        orders: list[list[int]] = []
        for l_idx, dim in enumerate(layer_dims):
            if 1 <= l_idx <= len(self.config.hidden_dims):
        ...
        # ------------------------------------------------------------------
        # determine which neurons have at least one significant edge
        # ------------------------------------------------------------------
        active: list[list[bool]] = []
        for l_idx, dim in enumerate(layer_dims):
            flags = [False] * dim
            if l_idx > 0:
                incoming = self.linear_layers[l_idx - 1].weight.detach().abs().cpu()
                if incoming.numel() > 0:
                    flags_in = (incoming >= threshold).any(dim=1).tolist()
                    for i, flag in enumerate(flags_in):
                        flags[i] = flags[i] or bool(flag)
            if l_idx < len(self.linear_layers):
                outgoing = self.linear_layers[l_idx].weight.detach().abs().cpu()
                if outgoing.numel() > 0:
        ...
        # ------------------------------------------------------------------
        # compute node positions with custom spacing
        # ------------------------------------------------------------------
        ...
        for l_idx, dim in enumerate(layer_dims):
            ...
            if l_idx == 0:  # input layer uses the full width
        ...
        # ------------------------------------------------------------------
        # draw edges using red colour intensity based on weight magnitude
        # ------------------------------------------------------------------
